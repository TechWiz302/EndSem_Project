        ...
        mycontract.methods.getPatient().call().then(async (res) => {
            for (let i = res.length - 1; i >= 0; i--) {
                if (res[i] === cookies['hash']) {
                    ...
                    const data = await response.json();
                    const alls = data.insurance;
                    const newList = alls.filter(ins => ins.policyNo !== policy);
                    data.insurance = newList;
                    ...
                    await mycontract.methods.addPatient(hash).send({ from: currentaddress }).then(() => {
                        setCookie('hash', hash);
                        alert("Deleted");
                        setInsurances(prevInsurances => prevInsurances.filter(ins => ins.policyNo !== policy));
                    }).catch((err) => {
                        console.log(err);
                    });
                }
            ...
            <div className='profile'>
                ...
                <div className="createInsuranceRecord">
                    <div className="signupbtn">
                        <button type='button' className="input-submit" onClick={() => { navigate("/dashboard/insuranceForm") }}>ADD Record</button>
                    </div>
                </div>
                        ...
                        <Table aria-label="simple table">
                            <TableHead>
                                <TableRow >
                                    ...
                                    <TableCell align="right" style={{ fontWeight: 'bolder' }}>Action</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {insurances.map((insurance, index) => (
                                    ...
                                    >
                                        ...
                                        <TableCell align="right" style={{ color: "#f54f4f" }}><DeleteIcon onClick={() => del(insurance.policyNo)} style={{ cursor: "pointer" }} /></TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
